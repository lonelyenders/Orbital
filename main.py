    ...
    class Game:
        ...
        def wall_generator(self):
            ...
            if len(self.all_walls) < 1:
                self.wall = Wall(random_wall_width, random_wall_height, settings.screen_width,
                                 self.ground.rect.top - random_wall_height + 50, random_wall_color)
    ...
    while True:
        ...
        if game.is_playing is True:
            ...
            if game.score_value > 0:
                game.game_title = game.font_big.render("Score : " + str(game.score_value), True, "white")

        else:
            # starting screen or game over screen with score
            settings.screen.blit(game.background_image, (0, 0))
            settings.screen.blit(game.play_button, game.play_button_rect)
            settings.screen.blit(game.game_title, game.game_title_rect)
